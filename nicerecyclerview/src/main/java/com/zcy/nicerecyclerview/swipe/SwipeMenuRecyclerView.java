...
import com.zcy.nicerecyclerview.swipe.touch.DefaultItemTouchHelper;
...
public class SwipeMenuRecyclerView extends RecyclerView {
    ...
    public void setAdapter(Adapter adapter) {
        if (adapter instanceof SwipeMenuAdapter) {
            setInnerAdapter((SwipeMenuAdapter) adapter);
        }
        if (adapter instanceof HeaderAndFooterWrapper) {
            ...
            if (innerAdapter instanceof SwipeMenuAdapter) {
                setInnerAdapter((SwipeMenuAdapter) innerAdapter);
            } else if (innerAdapter instanceof LoadMoreWrapper) {
                Adapter inner = ((LoadMoreWrapper) innerAdapter).getInnerAdapter();
                if (inner instanceof SwipeMenuAdapter) {
                    setInnerAdapter((SwipeMenuAdapter) inner);
                }
            }
        }
        if (adapter instanceof LoadMoreWrapper) {
            RecyclerView.Adapter innerAdapter = ((LoadMoreWrapper) adapter).getInnerAdapter();
            if (innerAdapter instanceof SwipeMenuAdapter) {
                setInnerAdapter((SwipeMenuAdapter) innerAdapter);
            } else if (innerAdapter instanceof HeaderAndFooterWrapper) {
                Adapter inner = ((HeaderAndFooterWrapper) innerAdapter).getInnerAdapter();
                if (inner instanceof SwipeMenuAdapter) {
                    setInnerAdapter((SwipeMenuAdapter) inner);
                }
            }
     ...
     * @param adapter
     */
    private void setInnerAdapter(SwipeMenuAdapter adapter) {
        adapter.setSwipeMenuCreator(mDefaultMenuCreator);
        adapter.setSwipeMenuItemClickListener(mDefaultMenuItemClickListener);
    }
